(** Secret key material kept outside the OCaml heap.
    ...
    (minor-to-major promotion and compaction copy heap blocks and leave the old
    copy behind, unzeroed). The buffer is zeroized when {!destroy} is called,
    ...
    - Copies spilled to the C stack or registers inside cryptographic
      primitives, or made by the kernel (page cache, socket buffers, hibernation
      images), or made by callers through [expose]/[unsafe_*].
...
(** [create ?hardened n] is a zero-filled secret of [n] bytes ([n >= 0]).
    ...
    [hardened] (default [false]) requests the page-backed tier: guard pages, a
    canary, [mlock], and exclusion from core dumps where the OS supports it.
    Each feature is {e best effort}; check {!val-status}. Cost: at least three
    OS pages of address space per secret. The default tier uses [calloc] memory
    with zero-on-release and is meant for many small or short-lived keys.

    Raises [Invalid_argument] if [n < 0] and [Out_of_memory] if allocation
    fails. *)
...
(** [random ?hardened n] is a secret of [n] bytes of OS entropy
    ...
    secret memory; no copy is made in the OCaml heap. When the platform has no
    OS source, the generator installed with {!set_entropy_source} is used (it
    writes into a {!Scratch} buffer that is wiped afterwards).

    @raise Entropy_unavailable when no source is available. *)

val of_string : ?hardened:bool -> string -> t
(** [of_string s] copies [s] into a new secret. The source string is immutable
    and {b cannot be wiped}: it stays in the OCaml heap until collected. Prefer
    {!random}, {!init}, {!of_bytes} or [Secret_unix.read_fd] when the secret can
    originate in controlled memory. *)

val of_bytes : ?hardened:bool -> wipe_source:bool -> bytes -> t
(** [of_bytes ~wipe_source b] copies [b]. With [~wipe_source:true] the source is
    zeroized afterwards with the same primitive as {!destroy}, including if
    allocation or copying raises. This cannot reach copies the GC may already
    have made of [b] (a [bytes] allocated in the minor heap is copied when
...
(** Constant-time equality of the contents (implemented in C). Lengths are
    compared first with an ordinary branch; the contents comparison takes time
    proportional to the length and independent of the values. There is
...
type lock =
  ...
  | `Failed of int
    (** [mlock] failed with this errno ([ENOMEM]: [RLIMIT_MEMLOCK]; [EPERM]: no
        [IPC_LOCK]) *)
  | `Lost_on_fork
    (** locked before a [fork]; locks are not inherited, see {!after_fork} *)
...
(** A {e copy} of a range (never an alias). Tier defaults to that of the source.
*)
...
(** [expose t f] calls [f] with a temporary copy of the contents in a {!Scratch}
    buffer (allocated directly in the major heap, so the minor collector never
...
(** Zero-copy views of the secret memory.
    ...
    heap, a representation the runtime supports and uses for static data). Any
...
module Unsafe : sig
  ...
  (** Borrowed bigarray view for the duration of [f] (for [Eqaf_bigstring],
      [Digestif.*.digest_bigstring], ...). After [f] the view is revoked (length
      0; accesses raise [Invalid_argument]). Sub-views created with
...
module Gc : sig
  ...
  (** Runs a minor collection and then zeroes the free part of the calling
      domain's minor heap, erasing the residue of every object that died young
      (handshake transients, KDF intermediates, scratch buffers) in this domain.
      Cost: one minor collection plus a [memset] of the minor heap (256k words
      by default). Other domains' minor heaps are not touched. Relies on
      documented runtime invariants of the domain state; it is hygiene, not a
      guarantee. *)
end
...
module Process : sig
  type feature =
    ...
    | `Not_dumpable
      (** Linux [prctl(PR_SET_DUMPABLE, 0)]: no core dumps, no [ptrace] by
          non-root, [/proc/pid/mem] hidden *)
    | `Lock_all
      (** [mlockall(MCL_CURRENT|MCL_FUTURE)]: no swapping of any page; subject
          to [RLIMIT_MEMLOCK] *)
  ...
  (** Applies the requested features (default: [`No_core_dump], [`Not_dumpable],
      [`Deny_attach]; [`Lock_all] is opt-in because it can fail or pin the whole
      heap) and returns what happened for each. *)

  val scrub_env : string -> bool
  (** [scrub_env name] zeroizes the value of the environment variable [name] in
      the process's [environ] block and removes the variable. Returns whether it
      was found. Copies already made by [Sys.getenv] are ordinary OCaml strings
