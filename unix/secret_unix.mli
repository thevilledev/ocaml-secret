(** Secrets and file descriptors.
    ...
    [Unix.read] copies through a 64 KiB buffer on the C stack and
    [In_channel] through a 64 KiB heap-allocated channel buffer; neither copy
    is ever zeroized. The functions here call [read(2)]/[write(2)] directly
    on the secret memory. The kernel page cache keeps its own copy of any
    file that was read; that is outside this library's reach. *)

val read : Unix.file_descr -> Secret.t -> off:int -> len:int -> int
(** [read fd t ~off ~len] reads at most [len] bytes into [t] at [off] with a
    single [read(2)] (retried on [EINTR]) and returns the number of bytes
    read (0 at end of file).
    @raise Unix.Unix_error on I/O errors.
...
(** Loops until [len] bytes were read.
