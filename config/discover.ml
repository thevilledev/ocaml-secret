  ...
  C.main ~name:"secret" (fun c ->
      ...
      let probes =
        if freestanding then []
          ...
          [
            probe ~name:"SECRET_HAVE_SECUREZEROMEMORY" ~includes:[ "windows.h" ]
              ~body:"char b[8]; SecureZeroMemory(b, sizeof b);";
            probe ~name:"SECRET_HAVE_BCRYPTGENRANDOM"
              ~includes:[ "windows.h"; "bcrypt.h" ]
              ~body:
                "unsigned char b[8]; (void)BCryptGenRandom(NULL, b, sizeof b, \
                 BCRYPT_USE_SYSTEM_PREFERRED_RNG);";
          ]
        else
          [
            probe ~name:"SECRET_HAVE_MEMSET_EXPLICIT" ~includes:[ "string.h" ]
            ...
            probe ~name:"SECRET_HAVE_ARC4RANDOM_BUF" ~includes:[ "stdlib.h" ]
              ~body:"unsigned char b[8]; arc4random_buf(b, sizeof b);";
            probe ~name:"SECRET_HAVE_MMAP" ~includes:[ "sys/mman.h" ]
              ~body:
                "void *p = mmap(NULL, 4096, PROT_READ|PROT_WRITE, \
                 MAP_PRIVATE|MAP_ANONYMOUS, -1, 0); (void)munmap(p, 4096);";
            probe ~name:"SECRET_HAVE_MLOCK" ~includes:[ "sys/mman.h" ]
              ~body:
                "char b[8]; (void)mlock(b, sizeof b); (void)munlock(b, sizeof \
                 b);";
            probe ~name:"SECRET_HAVE_MLOCKALL" ~includes:[ "sys/mman.h" ]
              ~body:"(void)mlockall(MCL_CURRENT | MCL_FUTURE);";
            probe ~name:"SECRET_HAVE_MADV_DONTDUMP" ~includes:[ "sys/mman.h" ]
              ~body:"int f = MADV_DONTDUMP; (void)f;";
            probe ~name:"SECRET_HAVE_MADV_NOCORE" ~includes:[ "sys/mman.h" ]
              ~body:"int f = MADV_NOCORE; (void)f;";
            probe ~name:"SECRET_HAVE_MADV_WIPEONFORK" ~includes:[ "sys/mman.h" ]
              ~body:"int f = MADV_WIPEONFORK; (void)f;";
            probe ~name:"SECRET_HAVE_MADV_KEEPONFORK" ~includes:[ "sys/mman.h" ]
              ~body:"int f = MADV_KEEPONFORK; (void)f;";
            probe ~name:"SECRET_HAVE_MAP_CONCEAL" ~includes:[ "sys/mman.h" ]
              ~body:"int f = MAP_CONCEAL; (void)f;";
            probe ~name:"SECRET_HAVE_PTHREAD_ATFORK" ~includes:[ "pthread.h" ]
              ~body:"(void)pthread_atfork(NULL, NULL, NULL);";
            probe ~name:"SECRET_HAVE_PRCTL_DUMPABLE" ~includes:[ "sys/prctl.h" ]
      ...
      in
      let defines =
        List.map (fun (name, ok) -> (name, C.C_define.Value.Switch ok)) probes
        @ [
