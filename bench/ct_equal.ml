...
let reps = try int_of_string Sys.argv.(2) with _ -> 200 (* calls per timed sample *)
let len = 64

let welch xs ys =
  let mean a = Array.fold_left ( +. ) 0. a /. float (Array.length a) in
  let var a m = Array.fold_left (fun s x -> s +. ((x -. m) ** 2.)) 0. a /. float (Array.length a - 1) in
...
let () =
  ...
  let run ?(raw = false) name contents_b =
    let name = if raw then name ^ " (C primitive)" else name ^ " (Secret.equal)" in
    let cls = Array.init n_samples (fun _ -> Random.bool ()) in
    let t0 = ref [] and t1 = ref [] in
    for i = 0 to n_samples - 1 do
      Secret.blit_from_string (if cls.(i) then contents_b else same) ~src_off:0 y ~dst_off:0 ~len;
      let s = ticks () in
      if raw then for _ = 1 to reps do ignore (Sys.opaque_identity (equal_raw fixed y)) done
      else for _ = 1 to reps do ignore (Sys.opaque_identity (Secret.equal fixed y)) done;
      let e = ticks () in
  ...
  for i = 0 to n_samples - 1 do
    ...
    let s = ticks () in
    for _ = 1 to reps do ignore (Sys.opaque_identity (String.equal same ys)) done;
    let e = ticks () in
    let d = Int64.to_float (Int64.sub e s) in
    if cls.(i) then t1 := d :: !t1 else t0 := d :: !t0
  done;
  let t = welch (trim !t0) (trim !t1) in
  Printf.printf "%-44s |t| = %6.2f (control: String.equal, expected to differ)\n" "differs in first byte (String.equal)" (abs_float t);
  Printf.printf "(%d samples x %d calls; |t| < 4.5 = no evidence of a timing difference)\n" n_samples reps
