...
let encrypt_traffic ~total key =
  ...
  for i = 0 to (total / record) - 1 do
    ...
    let ct = Mirage_crypto.AES.GCM.authenticate_encrypt ~key ~nonce:(Bytes.to_string nonce) msg in
...
let report name (v, w) =
  Printf.printf "  %-44s verbatim: minor=%-2d stack=%-2d other=%-2d | bswap32: minor=%-2d stack=%-2d other=%-2d\n%!"
...
let scenario ~reference ~title ~traffic ~mk_key =
  ...
  Secret.destroy w;
  let destroyed = scan_now ~reference ~excl "+ Secret.destroy (working secret)" in
  print_newline ();
...
let () =
  if not (supported ()) then (print_endline "leak scan: unsupported platform, skipped"; exit 0);
  Mirage_crypto_rng_unix.use_default ();
  let reference = Secret.random key_len in
  print_endline "Leak census: copies of a 32-byte AES-256 key (its 24-byte tail) in process memory.";
  print_endline "verbatim = as-is; bswap32 = byte-swapped per 32-bit word (how the generic AES key";
  print_endline "schedule stores the key). Copies inside Secret.t payloads are excluded.\n";
  let h0 = scan_now ~reference ~excl:[] "0. key held only in Secret.t" in
  print_newline ();
  let a_small =
    scenario ~reference ~traffic:(mib 1) ~title:"A. view: AES.GCM.of_secret (Secret.Unsafe.string_view k), schedule dies young"
      ~mk_key:(fun w -> Mirage_crypto.AES.GCM.of_secret (Secret.Unsafe.string_view w))
  in
  let a_large =
    scenario ~reference ~traffic:(mib 8) ~title:"A'. view: same, schedule promoted to the major heap"
      ~mk_key:(fun w -> Mirage_crypto.AES.GCM.of_secret (Secret.Unsafe.string_view w))
  in
  let b =
    scenario ~reference ~traffic:(mib 8) ~title:"B. baseline: AES.GCM.of_secret (Secret.unsafe_to_string k)"
      ~mk_key:(fun w -> Mirage_crypto.AES.GCM.of_secret (Secret.unsafe_to_string w))
  in
  let ok b = if b then "ok" else "FAIL" in
  let live_a, _, scrubbed_a, destroyed_a = a_small in
  let live_a', dropped_a', _, _ = a_large in
  let _, dropped_b, _, destroyed_b = b in
  let c1 = both h0 = 0 in
  let c2 = verbatim live_a = 0 && verbatim live_a' = 0 in
  let c3 = both scrubbed_a = 0 && both destroyed_a = 0 in
  let c4 = swapped dropped_a' > 0 in
  let c5 = verbatim dropped_b > 0 && verbatim destroyed_b > 0 in
  Printf.printf "Checks:\n";
  Printf.printf "  key only in Secret.t -> 0 copies: %s\n" (ok c1);
  Printf.printf "  view: the raw key never appears verbatim in the heap: %s\n" (ok c2);
  Printf.printf "  view + scrub: a key schedule that died young is erased: %s\n" (ok c3);
  Printf.printf "  view: a promoted key schedule survives in the major heap (needs the mirage-crypto change): %s\n" (ok c4);
  Printf.printf "  baseline: a heap-string key is never erased by the runtime: %s\n" (ok c5);
