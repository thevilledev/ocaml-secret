...
let test_survives_minor_gc () =
  ...
  Gc.minor ();
  Gc.minor ();
  Alcotest.(check string) "contents after minor" "promote me" (Secret.unsafe_to_string t);
  Alcotest.(check string) "view after minor" "promote me" v;
  Alcotest.(check bool) "view is not young" false (Secret.Scratch.is_young (Bytes.unsafe_of_string v))

let test_survives_compaction () =
  let secrets = List.init 2000 (fun i -> Secret.of_string (Printf.sprintf "secret-%05d" i)) in
  let views = List.map Secret.Unsafe.string_view secrets in
  (* churn the heap so compaction has work to do *)
  let junk = ref [] in
  for i = 0 to 50_000 do junk := Bytes.create (i mod 100 + 1) :: !junk done;
  junk := [];
  Gc.compact ();
  Gc.full_major ();
  List.iteri
...
let test_finalizer_releases () =
  ...
  let n = 20_000 in
  for _ = 1 to n do ignore (Secret.create 40) done;
  Gc.full_major ();
  Gc.full_major ();
  let live = Secret.live_count () in
  (* everything unreachable must have been finalized *)
  if live - base > 10 then Alcotest.failf "live_count %d (base %d) after full_major" live base;
  ...
  Alcotest.(check bool) "pool shrinks on reuse" true (Secret.pool_count () < pooled_before);
...
let test_gc_pressure_accounting () =
  ...
  let base = Secret.live_count () in
  for _ = 1 to 4000 do ignore (Secret.create 100_000) done;
...
let test_scrub_minor_heap () =
  ...
  Alcotest.(check string) "secrets unaffected" marker (Secret.unsafe_to_string t)
    ...
    [
      ( "gc",
        [
          ...
          Alcotest.test_case "survives compaction" `Quick test_survives_compaction;
          ...
          Alcotest.test_case "gc pressure accounting" `Quick test_gc_pressure_accounting;
