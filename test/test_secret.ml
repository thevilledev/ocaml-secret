...
let test_create_length () =
  ...
  Alcotest.check_raises "negative" (Invalid_argument "Secret.create: negative length")
    (fun () -> ignore (Secret.create (-1)))

let test_of_string_roundtrip () =
  let t = s_of "hello, secret" in
  Alcotest.(check string) "unsafe_to_string" "hello, secret" (Secret.unsafe_to_string t);
  Alcotest.(check bool) "equal_string" true (Secret.equal_string t "hello, secret");
  Alcotest.(check bool) "equal_string differs" false (Secret.equal_string t "hello, Secret");
  Alcotest.(check bool) "equal_string length" false (Secret.equal_string t "hello")
...
let test_destroy () =
  ...
  check_raises_destroyed "unsafe_to_string" (fun () -> Secret.unsafe_to_string t);
  ...
  Alcotest.(check bool) "status.destroyed" true (Secret.status t).Secret.destroyed;
  Alcotest.(check string) "pp" "<secret:destroyed>" (Format.asprintf "%a" Secret.pp t)

let test_pp () =
  let t = s_of "top secret" in
  Alcotest.(check string) "pp redacted" "<secret:10B>" (Format.asprintf "%a" Secret.pp t)
...
let test_blit_fill_sub () =
  ...
  Alcotest.(check string) "blit_from_string" "xxBCDxxx" (Secret.unsafe_to_string t);
  ...
  Secret.zero t;
  Alcotest.(check bool) "zero" true (Secret.equal_string t (String.make 8 '\000'));
  Alcotest.check_raises "sub bounds" (Invalid_argument "Secret.sub") (fun () ->
      ignore (Secret.sub t ~off:6 ~len:3));
  Alcotest.check_raises "blit bounds" (Invalid_argument "Secret.blit") (fun () ->
      Secret.blit ~src:t ~src_off:0 ~dst:u ~dst_off:2 ~len:4);
  ...
  Alcotest.(check string) "blit_from_bytes" "xxBCDxzz" (Secret.unsafe_to_string w)
...
let test_of_bytes_wipe () =
  ...
  Alcotest.(check string) "source wiped" (String.make 14 '\000') (Bytes.to_string b);
...
let test_expose () =
  ...
  let r =
    Secret.expose t (fun b ->
        captured := b;
        Alcotest.(check bool) "scratch not young" false (Secret.Scratch.is_young b);
        Bytes.to_string b)
  in
  Alcotest.(check string) "result" "exposed" r;
  Alcotest.(check string) "wiped after" (String.make 7 '\000') (Bytes.to_string !captured);
  (match Secret.expose t (fun _ -> failwith "boom") with
  | _ -> Alcotest.fail "expected exception"
  | exception Failure _ -> ());
  Alcotest.(check string) "wiped after raise" (String.make 7 '\000') (Bytes.to_string !captured)
...
let test_views () =
  ...
  if Sys.ocaml_version.[0] >= '5' then
    Alcotest.(check int) "Hashtbl.hash works" (Hashtbl.hash "View me") (Hashtbl.hash v);
...
let test_bigstring_view () =
  ...
  Secret.Unsafe.with_bigstring t (fun ba ->
      ...
      escaped := Some ba);
  Alcotest.(check string) "write through" "Bigstring" (Secret.unsafe_to_string t);
  (match !escaped with
  | None -> Alcotest.fail "no view"
  | Some ba ->
      Alcotest.(check int) "revoked dim" 0 (Bigarray.Array1.dim ba);
      Alcotest.check_raises "revoked access" (Invalid_argument "index out of bounds")
        (fun () -> ignore (Bigarray.Array1.get ba 0)))
...
let test_init_random () =
  ...
  if caps.Secret.os_random then begin
    ...
    Alcotest.(check bool) "random not zero" false
  ...
  Alcotest.(check bool) "fallback used only without OS entropy" (not caps.Secret.os_random) !hit

let test_with_secret () =
  let leaked = ref None in
  let r = Secret.with_secret 4 (fun t ->
      Secret.fill t 'w';
      leaked := Some t;
      Secret.unsafe_to_string t) in
  Alcotest.(check string) "result" "wwww" r;
  (match !leaked with
  | Some t -> Alcotest.(check bool) "destroyed after" true (Secret.is_destroyed t)
  | None -> Alcotest.fail "no secret");
  (match Secret.with_secret 4 (fun t -> leaked := Some t; failwith "x") with
  | _ -> Alcotest.fail "expected exception"
  | exception Failure _ -> ());
  match !leaked with
  | Some t -> Alcotest.(check bool) "destroyed after raise" true (Secret.is_destroyed t)
  | None -> Alcotest.fail "no secret"
...
let test_hardened () =
  ...
  Secret.fill t 'h';
  Alcotest.(check string) "usable" (String.make 48 'h') (Secret.unsafe_to_string t);
  let st = Secret.status t in
  Alcotest.(check bool) "page_backed iff supported" caps.Secret.hardened_tier st.Secret.page_backed;
  if st.Secret.page_backed then begin
    ...
    (match st.Secret.lock with
    | `Locked | `Failed _ -> ()
    | `Unsupported -> Alcotest.(check bool) "lock unsupported consistent" false caps.Secret.can_lock
    | `Lost_on_fork | `Not_requested -> Alcotest.fail "unexpected lock status");
    match st.Secret.no_core_dump with
    | `Yes -> Alcotest.(check bool) "nodump cap" true caps.Secret.can_exclude_from_dumps
    | `Unsupported -> ()
  ...
  Alcotest.(check bool) "sub inherits hardened" st.Secret.page_backed
  ...
  Alcotest.(check bool) "reuse usable" true
...
let test_pool_reuse () =
  ...
  Alcotest.(check bool) "reused block is zero" true
  ...
  Alcotest.(check int) "view length follows new owner" 97
...
let test_scratch () =
  ...
  let r = Secret.Scratch.with_ 5 (fun b -> kept := b; Bytes.fill b 0 5 'k'; 42) in
  Alcotest.(check int) "with_ result" 42 r;
  Alcotest.(check string) "with_ wiped" (String.make 5 '\000') (Bytes.to_string !kept);
  ...
  Alcotest.(check bool) "Bytes.create is young" true (Secret.Scratch.is_young (Bytes.create 10))

let test_capabilities () =
  let c = Secret.capabilities () in
  Alcotest.(check bool) "zeroize primitive named" true (String.length c.Secret.zeroize_primitive > 0);
  Alcotest.(check bool) "page size" true (c.Secret.page_size >= 0);
  if Sys.os_type = "Unix" then Alcotest.(check bool) "os random on unix" true c.Secret.os_random
...
let test_process () =
  ...
  if Sys.os_type = "Unix" then begin
    ...
    Alcotest.(check bool) "found" true (Secret.Process.scrub_env "SECRET_TEST_VAR");
    Alcotest.(check bool) "removed" true (Sys.getenv_opt "SECRET_TEST_VAR" = None);
    Alcotest.(check bool) "not found twice" false (Secret.Process.scrub_env "SECRET_TEST_VAR")
  end;
  ...
  | [ (`No_core_dump, Error (`Failed e)) ] -> Alcotest.failf "setrlimit failed: %d" e
    ...
    [
      ( "api",
        [
          ...
          Alcotest.test_case "compare/hash/marshal" `Quick test_compare_hash_marshal;
          ...
          Alcotest.test_case "status default tier" `Quick test_status_default_tier;
